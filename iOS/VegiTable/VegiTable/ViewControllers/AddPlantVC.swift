...
extension AddPlantVC: Downloadable {
    func didReceiveData(data: Any) {
        DispatchQueue.main.sync {
            // plant returned after its been added
            print(data)
        }
    }
    ...
    @IBAction func AddPlant() {
        ...
        // do form validation
        let imageUrl = inputImageUrl.text!
        let newPlant: [String: Any] = [
            PlantColNames.plantName.rawValue : inputPlantName.text!,
            PlantColNames.plantType.rawValue : inputPlantType.text!,
            PlantColNames.temperatureMin.rawValue : inputTemperatureMin.text!,
            PlantColNames.temperatureMax.rawValue : inputTemperatureMax.text!,
            PlantColNames.phMin.rawValue : inputPHMin.text!,
            PlantColNames.phMax.rawValue : inputPHMax.text!,
            PlantColNames.ppmMin.rawValue : inputPPMMin.text!,
            PlantColNames.ppmMax.rawValue : inputPPMMax.text!,
            PlantColNames.lightMin.rawValue : inputLightMin.text!,
            PlantColNames.lightMax.rawValue : inputLightMax.text!,
            PlantColNames.humidityMin.rawValue : inputHumidityMin.text!,
            PlantColNames.humidityMax.rawValue : inputHumidityMax.text!,
            PlantColNames.plantPhase.rawValue : inputGrowthPhase.text!,

            //PlantColNames.bucketId_fk.rawValue : Int(inputBucket),
            PlantColNames.bucketId_fk.rawValue : 35,
            PlantColNames.imageURL.rawValue : imageUrl
        ]
        
        model.createPlant(newPlant: newPlant, url: ApiCalls.createPlant)
    }
}


class AddPlantVC: UIViewController, UITextFieldDelegate {

    // used for api functions
    let model = PlantsModel()
